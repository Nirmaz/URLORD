...
def add_data_to_storage_train(storage_dict, subject_id, subject_data):
...
def add_data_to_storage_test(storage_dict, subject_id, subject_data):
    storage_dict[subject_id] = {}
...
def create_and_store_training_patches(case: np.array,
                            patchs_storage: dict ,subject_id: str ,
                            model_original_dim: Tuple[int, int, int] = (64, 64, 20),
                            patch_stride: Tuple[int, int, int] = (32, 32, 28), with_without_body:bool = True, min_num_of_body_voxels:int = 64*64*5):
    """
    ...
    # plt.show()
    for (patches, patches_coords) in [(patches1, patches_coords1)]:
        ...
        for j, patch in enumerate(patches):
            ...
            body_pixels = np.zeros((patch.shape[0], patch.shape[1], patch.shape[2]))
            body_pixels[patch > 0.1] = 1
            num_of_body_voxels = body_pixels.sum()
            print(num_of_body_voxels)
            if with_without_body and num_of_body_voxels < min_num_of_body_voxels:
                continue

            patchs_storage[f'{subject_id}_{current_x_coord}_{current_y_coord}_{current_z_coord}'] = patch
            if j == 10:
                plt.title(f'{j}_{current_x_coord}_{current_y_coord}_{current_z_coord}')
                print(j, np.unique(patch[:,:, 0]), "unique patch")
...
def write_image_data_to_file(image_files, data_storage, subject_ids, scale=None, preproc=None, rescale_res=None, metadata_path=None, default_res= [1.56,1.56,3], train = True):
    ...
    for subject_id, set_of_files in zip(subject_ids, image_files):
        ...
        if train:
            add_data_to_storage_train(data_storage, subject_id, subject_data)
        else:
            add_data_to_storage_test(data_storage, subject_id, subject_data)
...
def write_data_to_file(training_data_files, out_file,out_file_patchs ,subject_ids = None, normalize='all', scale=None, preproc=None, rescale_res=None, metadata_path=None, train = True, store_patches = True):
    """
    ...
    data_dict = {}
    patchs_dict = {}
    write_image_data_to_file(training_data_files, data_dict, subject_ids, scale = scale, preproc = preproc, rescale_res = rescale_res, metadata_path = metadata_path, train = train)
    ...
    if store_patches:
        ...
        for subject_id in data_dict.keys():
            if train:
                ...
                subject_data =  data_dict[subject_id]['data']
            plt.title("subject data")

            # plt.imshow(subject_data[:,:,0], cmap='gray')
            # plt.show()
            create_and_store_training_patches(subject_data , patchs_dict, subject_id)
...
def create_load_hdf5(normalization, data_dir, scans_dir, train_modalities, ext, overwrite = False, preprocess=None, scale= None, rescale_res=None, metadata_path=None, train = True, store_patches = True):
    ...
    if overwrite or not os.path.exists(data_file):
        ...
        _, (mean, std) = write_data_to_file(training_files, data_file, data_file_patchs, subject_ids=subject_ids, normalize=normalization, preproc=preproc_func, scale = scale, rescale_res = rescale_res, metadata_path = metadata_path, train = train, store_patches = store_patches)
...
if __name__ == '__main__':
    create_load_hdf5(normalization = "all", data_dir = '/cs/casmip/nirm/embryo_project_version1/DATA-AFTER/placenta' ,scans_dir = '/cs/casmip/nirm/embryo_project_version1/DATA-RAW/placenta', train_modalities = [ "volume", "truth"], ext ="",overwrite =True, preprocess = "window_1_99", scale = None, train = True, store_patches= True)
    create_load_hdf5(normalization = "all", data_dir = '/cs/casmip/nirm/embryo_project_version1/DATA-AFTER/TRUFI' , scans_dir = '/cs/casmip/nirm/embryo_project_version1/DATA-RAW/TRUFI', train_modalities = [ "volume", "truth"], ext = "", overwrite = True, preprocess = "window_1_99", scale = [0.5, 0.5, 1],rescale_res = [1.56,.56,3], metadata_path = None, train = True, store_patches= True)
