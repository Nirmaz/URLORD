...
class EmbSDNetExecutor(Executor):
    """
    Executor for training SDNet.
    """

    def __init__(self, conf, model):
        super(EmbSDNetExecutor, self).__init__(conf, model)
        self.model = model
    ...
    def test(self):
        ...
        if self.conf.modality == 'all':
            ...
            super(EmbSDNetExecutor, self).test()
    ...
    def _init_labelled_data_generator(self):
        ...
        log.info('hereeeee')
        self.data = self.loader.load_labelled_data(self.conf.split, 'training', modality=self.conf.modality, downsample = self.conf.image_downsample)
        log.info('outofhere')
        log.info('data_images', self.data.images)
        self.conf.data_len = self.data.size()
        datagen_dict1 = self.get_datagen_params()
        datagen_dict2 = self.get_datagen_params()
        # log.info('data_images', self.data.images.shape)
    ...
    def _init_unlabelled_data_generator(self):
        ...
        return ImageDataGenerator(**datagen_dict).flow(ul_data.images, batch_size=self.conf.batch_size, seed=self.conf.seed)
    ...
    def init_image_callback(self):
        ...
        if self.gen_labelled is None:
            ...
            self.data = self.loader.load_labelled_data(self.conf.split, 'training', self.conf.modality, downsample = self.conf.image_downsample)
