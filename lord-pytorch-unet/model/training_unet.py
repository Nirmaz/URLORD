...
from model.modules_unet import VGGDistance, ULordModel2D, ULordModel3D, UNet3D, UNet2D
...
class Lord:
	...
	def load(self, model_dir, model_id, num_exp = None, path_exp = None):
		...
		elif model_id == 'ULord2D':
			self.ulord_model = ULordModel2D(self.config, self.config_unet)
			self.ulord_model .load_state_dict(torch.load(os.path.join(model_dir, 'ulord.pth')))

		if model_id == 'UNet3D':
			self.ulord_model = UNet3D(self.config_unet)
			self.ulord_model.load_state_dict(
				torch.load(os.path.join(model_dir, 'ulord.pth')))

		elif model_id == 'UNet2D':
			self.ulord_model = UNet2D(self.config_unet)
			self.ulord_model.load_state_dict(
				torch.load(os.path.join(model_dir, 'ulord.pth')))
	...
	def evaluate(self, path_to_weights ,param_data,data_with_gt, path_result):
		...
		post_processing = PostProcessingSeg(self.ulord_model ,param_data['min_val'], param_data['max_val'], data_with_gt, param_data['margin'],self.config['train']['batch_size'], param_data['patch_stride'], param_data['model_original_dim'], bb = False)
	...
	def do_step_ulord(self, epoch, recon_decay, batch, optimizer, criterion, seg_decay, class_decay, content_decay, scheduler, train_loss, dice_loss_epoch_train, dice_loss_epoch_witha, reco_loss_epoch, reco_loss_epoch_witha):
		# print(batch, 'batch')
		...
		reco_loss = criterion(out['img'], batch_t['img']) + criterion(
			out['img_u'], batch_u['img'])
	...
	def train(self,model_id,  imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir, loaded_model):
		print("arrive train")
		if model_id == 'ULord2D':
			self.train_ULordModel2D( imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir, loaded_model)

		elif model_id == 'ULord3D':
			self.train_ULordModel3D(imgs, segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir,loaded_model)

		elif model_id == 'UNet3D':
			print("arrive location")
			self.train_UNet3D(imgs, segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir,loaded_model)

		elif model_id == 'UNet2D':
			self.train_UNet2D(imgs, segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir,loaded_model)



	def train_ULordModel2D(self,  imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir, loaded_model):
	...
	def train_UNet2D(self,  imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir, loaded_model):
		self.imgs, self.segs, self.classes, self.imgs_u, self.segs_u, self.classes_u, self.imgs_v, self.segs_v, self.classes_v, self.imgs_t, self.segs_t, self.classes_t = imgs, segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t
		self.build_data_set_2d()
		self.rotation_transform = MyRotationTransform(angles=[-90, 0, 90])
		if not loaded_model:
	...
	def train_UNet3D(self,  imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t, model_dir, tensorboard_dir, loaded_model):
		print("enter UNet3D")
		self.imgs, self.segs, self.classes, self.imgs_u, self.segs_u, self.classes_u, self.imgs_v, self.segs_v, self.classes_v, self.imgs_t, self.segs_t, self.classes_t =  imgs ,segs, classes, imgs_u, segs_u, classes_u, imgs_v, segs_v, classes_v, imgs_t, segs_t, classes_t
		self.build_data_set_3d()
		self.rotation_transform = MyRotationTransform3D(angles=[-90, 0, 90])
	...
	def train_ULordModel(self, model_dir, tensorboard_dir):
		...
		for epoch in range(self.config['train']['n_epochs']):
			...
			for i,  batch in enumerate(self.train_iter):
				# print(batch, 'batch')
				start_time_preprocees = time.time()
				self.do_step(epoch, recon_decay, batch, self.optimizer, criterion, seg_decay, class_decay, content_decay, scheduler, train_loss, dice_loss_epoch_train, dice_loss_epoch_witha, reco_loss_epoch, reco_loss_epoch_witha)
				# print(recon_decay, "recon_decay")
				# if epoch % 10 == 0 and epoch > 0 and i == 2:
				# 	print("here recon decay")
				# 	if recon_decay >= 20:
	...
	def generate_samples_ulord2d(self, dataset, epoch ,path_result, shape ,  n_samples=5, randomized = False):
		...
		with torch.no_grad():
			...
			self.generate_rec_seg_samples2d(path_results_dice, epoch,samples, cloned_all_samples, n_samples, path_results_output, randomized, recon_loss = self.g_rec)
	...
	def generate_samples_ulord3d(self, dataset, epoch ,path_result, shape ,  n_samples = 5, randomized = False):
		...
		with torch.no_grad():
			...
			self.generate_rec_seg_samples3d(path_results_dice, epoch, samples,cloned_all_samples, n_samples,path_results_output, rnd,num_slice, shape, recon_loss = self.g_rec)
	...
	def generate_rec_seg_samples2d(self,path_results_dice, epoch, samples,cloned_all_samples, n_samples,path_results_output, randomized, recon_loss = True):
	...
	def generate_rec_seg_samples3d(self,path_results_dice, epoch, samples,cloned_all_samples, n_samples,path_results_output, rnd,num_slice, shape, recon_loss = True):
